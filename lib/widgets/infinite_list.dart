  ...
  Widget buildPostsView() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Flexible(child: _cardChargerList(description!, avaliable, match, types, private, price, direction, description2, id!, latitude, longitude, rate!));
      });
  }

  //funcion respectiva a la card de los cargadores
  Widget _cardChargerList(String description, bool avaliable, bool match, List<ConnectionType> types, bool private, double  price, String direction, String description2, int id, double latitude, double longitude, double rate) {
    return GestureDetector(
      onTap: () {
        GoRouter.of(context)
            .go('/chargers/$id');//Navigator.push(context, MaterialPageRoute(builder: (context) => ChargerInfo()));
      },
      child: CardInfoWidget(location: description, rating: rate, types: types, available: avaliable, match: match, private: false, price: price, direction: direction, description: description2, private_list: private, latitude: latitude, longitude: longitude, id: id),
    );
